...
for round in range(1, 25):
    ...
    for idx, row in results.iterrows():
        ...
        if the_driver not in drivers:
            drivers[the_driver] = { 'name' : row['FullName'], 'team' : row['TeamName'], 'results' : [], 'delta' : [], 'pos' : [] }
            name_abbrev[row['FullName']] = the_driver
    ...
    for team, team_data in results.groupby("TeamName"):
        ...
        drivers[driver1['Abbreviation']]['pos'].append(driver1['Position'])
        drivers[driver2['Abbreviation']]['pos'].append(driver2['Position'])

        driver_teams[team] = [driver1['FullName'], driver2['FullName']]
...
try:
    with open('./driver_teams.json', 'w', encoding='utf-8') as f:
        json.dump(driver_teams, f, indent=4) # Using indent for human-readable formatting
    print(f"Data successfully dumped to {'driver_teams.json'}")
except IOError as e:
    print(f"Error writing to file: {e}") 

'''
try:
    with open('./name_abbrev.json', 'w', encoding='utf-8') as f:
        json.dump(name_abbrev, f, indent=4) # Using indent for human-readable formatting
